"""Build the Thai 700-case score-disambiguation training set."""
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/th_curated_700"))
    ...
    if len(rows) != 820:
        raise SystemExit(f"expected 820 authored rows, got {len(rows)}")
    ...
    if len(splits["train"]) != 700 or any(
